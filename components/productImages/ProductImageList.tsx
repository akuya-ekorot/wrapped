...
import { type ProductImage, CompleteProductImage } from "@/lib/db/schema/productImages";
...
export default function ProductImageList({
  ...
  const { optimisticProductImages, addOptimisticProductImage } = useOptimisticProductImages(
    productImages,
    images,
  products 
  );
  const [open, setOpen] = useState(false);
  const [activeProductImage, setActiveProductImage] = useState<ProductImage | null>(null);
  const openModal = (productImage?: ProductImage) => {
    setOpen(true);
    productImage ? setActiveProductImage(productImage) : setActiveProductImage(null);
  };
  ...
  return (
    <div>
      <Modal
        open={open}
        setOpen={setOpen}
        title={activeProductImage ? "Edit ProductImage" : "Create Product Image"}
      >
        <ProductImageForm
  ...
  return (
    ...
    >
      ...
      <Button variant={"link"} asChild>
        <Link href={ basePath + "/" + productImage.id }>
          Edit
        </Link>
      </Button>
    </li>
    ...
    <div className="text-center">
      ...
      <div className="mt-6">
        <Button onClick={() => openModal()}>
          <PlusIcon className="h-4" /> New Product Images </Button>
      </div>
    </div>
  );
};
