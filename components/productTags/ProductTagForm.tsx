...
import { type ProductTag, insertProductTagParams } from "@/lib/db/schema/productTags";
import {
...
const ProductTagForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const productTagParsed = await insertProductTagParams.safeParseAsync({ tagId,
  productId, ...payload });
        ...
        <Select defaultValue={productTag?.tagId} name="tagId">
          ...
          {tags?.map((tag) => (
            <SelectItem key={tag.id} value={tag.id.toString()}>
              {tag.id}{/* TODO: Replace with a field from the tag model */}
            </SelectItem>
           ))}
          </SelectContent>
        </Select>
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            closeModal && closeModal();
            startMutation(async () => {
              addOptimistic && addOptimistic({ action: "delete", data: productTag });
