...
import { integer, real, varchar, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const orderItems = pgTable('order_items', {
  ...
  variantId: varchar("variant_id", { length: 256 }).references(() => variants.id).notNull(),
  orderId: varchar("order_id", { length: 256 }).references(() => orders.id, { onDelete: "cascade" }).notNull(),
...
export const insertOrderItemSchema = createInsertSchema(orderItems).omit(timestamps);
export const insertOrderItemParams = baseSchema.extend({
...
export const updateOrderItemParams = baseSchema.extend({
  quantity: z.coerce.number(),
  amount: z.coerce.number(),
  variantId: z.coerce.string().min(1),
  orderId: z.coerce.string().min(1)
...
export type CompleteOrderItem = Awaited<ReturnType<typeof getOrderItems>>["orderItems"][number];
