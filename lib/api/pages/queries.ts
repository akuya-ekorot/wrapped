...
export const getPageByIdWithContentBlocks = async (id: PageId) => {
  const { id: pageId } = pageIdSchema.parse({ id });
  const rows = await db
    .select({ page: pages, contentBlock: contentBlocks })
    .from(pages)
    .where(eq(pages.id, pageId))
    .leftJoin(contentBlocks, eq(pages.id, contentBlocks.pageId));
  if (rows.length === 0) return {};
  const p = rows[0].page;
  const pc = rows
    .filter((r) => r.contentBlock !== null)
    .map((c) => c.contentBlock) as CompleteContentBlock[];
