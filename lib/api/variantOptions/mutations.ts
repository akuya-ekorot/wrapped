...
export const createVariantOption = async (variantOption: NewVariantOptionParams) => {
  const newVariantOption = insertVariantOptionSchema.parse(variantOption);
  try {
    const [v] =  await db.insert(variantOptions).values(newVariantOption).returning();
...
export const updateVariantOption = async (id: VariantOptionId, variantOption: UpdateVariantOptionParams) => {
...
export const deleteVariantOption = async (id: VariantOptionId) => {
  const { id: variantOptionId } = variantOptionIdSchema.parse({ id });
  try {
    const [v] =  await db.delete(variantOptions).where(eq(variantOptions.id, variantOptionId!))
    .returning();
    return { variantOption: v };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
